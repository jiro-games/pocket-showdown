...
export function PokemonCard({ card, scale }: PokemonCardProps) {
  ...
  const [usingFallbackArtwork, setUsingFallbackArtwork] = useState(false);
  ...
  const handleArtworkError = (e: React.SyntheticEvent<HTMLImageElement, Event>) => {
    if (isExCard) {
      e.currentTarget.src = `/assets/pokemon/${pokemonNameForArtwork}.png`;
      setUsingFallbackArtwork(true);
    }
  };

  return (
    <div
      className={`pokemon__wrapper ${isExCard ? 'card__content--ex' : ''}`}
      style={{
        transform: `scale(${scale})`,
      }}
    >
      <img
        className={`pokemon__image ${usingFallbackArtwork ? 'pokemon__image--fallback' : ''}`}
